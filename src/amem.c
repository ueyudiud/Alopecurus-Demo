...
a_mem aloM_realloc(alo_State T, a_mem oldblock, size_t oldsize, size_t newsize) {
...
#endif
	a_mem newblock = G->alloc(G->context, oldblock, oldsize, newsize);
...
void aloM_free(alo_State T, a_mem oldblock, size_t oldsize) {
	aloE_assert(!oldblock == !oldsize, "block size mismatch");
	Gd(T);
...
size_t aloM_adjsize(alo_State T, size_t capacity, size_t require, size_t limit) {
	if (require > limit) {
		aloU_szoutoflim(T);
	}
...
size_t aloM_growsize(alo_State T, size_t capacity, size_t limit) {
	size_t newcap = capacity * 2;
	if (newcap > limit) {
		if (capacity >= limit) {
			aloU_szoutoflim(T);
