...
#define aloB_check(T,b,e...) (aloE_assert((b).instk == (b).buf || (b) == (T)->memstk.top, "buffer not on the top of stack"), ##e)

#define aloB_close(T,b) aloB_check(T, b, ({ if ((b).instk != (b).buf) aloB_bcloseaux(T, &(b)); }))
...
#define aloB_putc(T,b,ch) ((b)->data.len < (b)->data.cap ? aloB_rputc(b, ch) : (aloB_bgrow_(T, b), aloB_rputc(b, ch)))
#define aloB_rputc(b,ch) aloE_void(aloB_check(T, b, (b)->buf[(b)->data.len++] = aloE_byte(ch)))
